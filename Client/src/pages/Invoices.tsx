...
const Invoices: React.FC = () => {
  ...
  return (
    <div className="page invoices-page">
      ...
      {/* Modern Card Header */}
      <div className="invoices-header-card" style={{marginBottom:'2rem', background:'linear-gradient(135deg, #1e40af 60%, #1e3a8a 100%)', color:'#fff', borderRadius:'18px', padding:'2.5rem 2rem 1.5rem 2rem', boxShadow:'0 4px 24px rgba(30,64,175,0.10)'}}>
        <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'2em'}}>
          <div>
            <div className="invoices-title" style={{fontSize:'2.1rem', fontWeight:700, marginBottom:'.2em', letterSpacing:'-1px'}}>Invoices</div>
            <div className="invoices-subtitle" style={{fontSize:'1.1rem', color:'#e0e7ef', fontWeight:400}}>Manage and track your sales invoices</div>
          </div>
          {/* Add button if needed */}
        </div>
      </div>
      {/* Controls Row */}
      <div className="invoices-controls" style={{display:'flex', flexWrap:'wrap', gap:'2em', alignItems:'center', marginBottom:'2em'}}>
      ...
      <Modal
        isOpen={isDeleteModalOpen && selectedInvoices.length > 0}
        onClose={() => setIsDeleteModalOpen(false)}
        title="Delete Selected Invoices"
      >
        <div style={{textAlign:'center', padding:'2em 1em'}}>
          <Trash2 size={40} style={{color:'var(--error-500)', marginBottom:8}} />
          <p>Are you sure you want to delete <b>{selectedInvoices.length}</b> selected invoice(s)?</p>
          <p>This action cannot be undone.</p>
          <div style={{display:'flex', justifyContent:'center', gap:'1em', marginTop:'2em'}}>
            <Button type="button" onClick={() => setIsDeleteModalOpen(false)}>
              Cancel
            </Button>
            <Button type="button" className="danger" onClick={handleDeleteMultipleInvoices}>
              Delete
            </Button>
          </div>
        </div>
